...
select * from office where rollno = any 
(select rollno from empdetails where age=26)
...
SELECT ProductName, UnitPrice * (profit + IFNULL(loss, 0))
FROM Products;
...
WHERE salary = 40000 ;
use offices
select IFNULL(place, "hello") from empdetails
select * from emplist
create table emplist (rollno int , empname varchar(50), age int, phoneno varchar(20), address varchar(100), place varchar(50))
insert into emplist 
select * from empdetails where age =26
